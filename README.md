...
The page is currently set so that 2 or 3 adjacent nodes are needed for a node to come alive regardless of whether the node is currently alive or dead, because this creates interesting results. To watch the algorithm play out, click the button that says "Run". The speed slider sets how many iterations run per second, and the "Step" button advances a single iteration. The "Run" button can be toggled to pause and resume the game. The whole grid can be cleared or reset to the starting shape, and individual nodes can be toggled as "dead" or "alive".
